<template>
  ...
  >
    <div class="modal-dialog modal-lg modal-width">
      <Form @submit="Submit" :validation-schema="Schema" ref="addPlayerFormRef">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="addPlayerModalLabel">Add {{ position }}</h5>
            <button
              type="button"
              class="btn-close"
...
export default {
  ...
  methods: {
    ...
    ClearModal() {
      this.$refs.addPlayerFormRef.resetForm();
    ...
    Submit() {
      ...
      this.$axios
        .post("/api/Player/create", this.player)
        .then(() => {
          this.$emit("get");
